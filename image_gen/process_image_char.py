...
def export_verilog(color_indices, palette_rgb12, grid_cols, grid_rows, output_filename, module_name):
    with open(output_filename, 'w') as f:
        ...
        f.write("    parameter COLOR_WIDTH = 4,\n")
        ...
        f.write(f"localparam CHAR_WIDTH_X = {grid_cols};\n")
        f.write(f"localparam CHAR_WIDTH_Y = {grid_rows};\n")

        f.write(f"(* rom_style = \"block\" *) reg [CHAR_WIDTH_X * CHAR_WIDTH_Y * COLOR_WIDTH - 1:0] pixel_map = {{\n")
        for idx, color_idx in enumerate(color_indices):
            if idx % grid_cols == 0 and idx != 0:
                f.write("\n")
            if idx == len(color_indices) - 1:
                f.write(f"    4'h{color_idx:X}\n")
            else:
                f.write(f"    4'h{color_idx:X}, ")
        ...
        f.write("        rgb_id <= 4'hB;\n")
        f.write("    end else if (char_on) begin\n")
        f.write("        rgb_id <= pixel_map[(char_y_rom * CHAR_WIDTH_X + char_x_rom) * COLOR_WIDTH +: COLOR_WIDTH];\n")
